  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        setData(events);
        setList(events);
        setTotalSolicitudes(querySnapshot.size); // 
  ...
  const onLoadMore = async () => {
    ...
    try {
      ...
      const querySnapshot = await getDocs(
        collection(db, 'eventos').startAfter(ultimoEvento).limit(5) // Obtener 5 solicitudes más
      );
      ...
      for (const docRef of querySnapshot.docs) {
        ...
        if (evento.estado === 'activo') { // Filter events with estado "inactivo"
